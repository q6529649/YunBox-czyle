...
function kadima_customizer( $wp_customize ) {
	...
	for( $ci=1; $ci<=5; $ci++ ) {
		...
		$wp_customize->add_setting(
		   'kadima_options[custom_icons_'.$ci.']',
			array(
				'default'			=> esc_attr($wl_theme_options['custom_icons_'.$ci]),
				'type'				=> 'option',
				'capability'		=> 'edit_theme_options',
				'sanitize_callback'	=> 'kadima_sanitize_text',
			)
		...
		$wp_customize->add_setting(
		   'kadima_options[custom_img_'.$ci.']',
			array(
				'default'			=>esc_attr($wl_theme_options['custom_img_'.$ci]),
				'type'				=>'option',
				'capability'		=>'edit_theme_options',
				'sanitize_callback'	=>'kadima_sanitize_text',
			)
		...
		$wp_customize->add_control(
			...
			array(
				'label'        	=> __( '自定义模块 '.$count12[$ci-1].' 图标', 'kadima' ),
				'description'	=> __('','kadima'),
				'section'  		=> 'custom_section_'.$ci,
				'type'			=> 'text',
				'settings'   	=> 'kadima_options[custom_icons_'.$ci.']'
			)
		);
		$wp_customize->add_control(
			new WP_Customize_Image_Control(
				$wp_customize,
				'kadima_custom_img_'.$ci,
				array(
					'label'      => __( '自定义模块 '.$count12[$ci-1].' 图片', 'kadima' ),
					'section'    => 'custom_section_'.$ci,
					'settings'   => 'kadima_options[custom_img_'.$ci.']'
				)
			)
		);
		for( $i=1; $i<=12; $i++ ) {
